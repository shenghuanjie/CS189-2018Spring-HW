    ...
    while it < max_iterations and above_threshold:
        ...
        for j in range(len(w_guesses)):
            if gd:
                ...
                Xcopy = X.copy()
                ycopy = y.copy()
                sample_idxes = np.random.choice(n, n, replace=False)
                Xcopy = Xcopy[sample_idxes, :]
                ycopy = ycopy[sample_idxes, :]

                for i_mini in range(nbatch):
                    start, end = int(i_mini * batch_size), int((i_mini + 1) * batch_size)
                    sampleX = Xcopy[start:end, :]
                    sampleY = ycopy[start:end, :]
...
plt.legend()
#plt.show()
plt.savefig('Figure_2f_mini-1.png')
plt.close()
...
plt.legend()
#plt.show()
plt.savefig('Figure_2f_mini-2.png')
plt.close()
...
plt.legend()
#plt.show()
plt.savefig('Figure_2f_mini-3.png')
plt.close()
