...
def plot_differences(iterations, diff_mu_ga, diff_mu_em, mu, n_examples):
    ...
    plt.loglog(np.arange(iterations), diff_mu_ga, c='r', label='GD')
    plt.loglog(np.arange(iterations), diff_mu_em, c='b', label='EM')
    plt.legend()
    ...
    # plt.show()
    plt.savefig(whichPart + 'difference_loglog.png')
    plt.close()
...
# Set this to True if you want to visualize data
