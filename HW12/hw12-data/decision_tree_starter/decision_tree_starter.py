...
if __name__ == "__main__":
    ...
    for dataset in all_datasets:
        ...
        if dataset == 'spam':
            ...
            print('\\textbf{{The top ' + str(print_n) + ' easiest samples to classify}}')
            ...
            print()
            print('\\textbf{{The top ' + str(print_n) + ' hardest samples to classify}}')
            for ip in range(print_n):
                print(str(ip+1) + '. ' + str(class_names[y[sorted_indices[-ip]]]) + ': ' +
                      str([x for x in zip(features, X[sorted_indices[-ip], :]) if x[1] > 0]))
        ...
        if dataset == 'titanic':
            ...
            test_hat = np.array(test_hat, dtype=int)
